...
#define DEBUG_SIGNAL_PROC false
...
float SINC_128[128] = {
    0.017232, 0.002666, -0.013033, -0.026004, -0.032934, -0.031899, -0.022884, -0.007851, 0.009675, 0.025427,
...
Pedal::Pedal(int input_pin_1, int input_pin_2, unsigned long millis, int conversion_rate)
    : input_pin_1(input_pin_1), input_pin_2(input_pin_2),
      previous_millis(millis), conversion_rate(conversion_rate), fault(false)
{
    // Init pins
...
void Pedal::pedal_update(unsigned long millis) {
    ...
    if (millis - previous_millis > conversion_period) {
        ...
        final_pedal_value = pedal_filtered_1;
...
void Pedal::pedal_can_frame_update(can_frame *tx_throttle_msg)
{
    float throttle_volt = (float)final_pedal_value * APPS_PEDAL_1_RANGE / 1024; // Converts most update pedal value to a float between 0V and 5V (APPS_PEDAL_1_RANGE)
    ...
    */
    ...
    else if (throttle_volt < MAX_THROTTLE_IN_VOLT)
    {
        ...
        //     Serial.print("Output voltage too small:");
        //     Serial.print(" ");
        //     Serial.print(throttle_volt);
        //     Serial.print(" ");
        //     Serial.println(throttle_torque_val);
